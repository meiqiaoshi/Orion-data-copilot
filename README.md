...
Run from the repository root (**Swagger UI** at **`/docs`**, **ReDoc** at **`/redoc`**, **`/openapi.json`** — routes are grouped under **probes**, **v1**, and **service**. **`/v1/plan`**, **`/v1/query`**, and **`/v1/version`** declare **`security`** so **Authorize** works for **`X-API-Key`** or **Bearer**; leave it empty when the server has no **`ORION_API_KEY`**):
...
- **Optional auth:** set **`ORION_API_KEY`** in the environment. When set, **`/v1/plan`**, **`/v1/query`**, and **`/v1/version`** require header **`X-API-Key: <key>`** or **`Authorization: Bearer <key>`**; otherwise the server returns **`401`** with JSON **`{"detail": "…"}`** (see **`/docs`** / **`/openapi.json`**). Omit the variable for local development (no key required).
- **CORS:** default **`ORION_CORS_ORIGINS`** is **`*`** (wildcard). Set a comma-separated allowlist (e.g. `https://myapp.example,http://127.0.0.1:8501`) to restrict browser origins; restart the server after changing it.
